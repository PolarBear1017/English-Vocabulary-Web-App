...
const FolderDetail = ({
  ...
  const viewingIndex = viewingWord
    ? sortedActiveFolderWords.findIndex((word) => word.id === viewingWord.id)
    : -1;
  const previousWord = viewingIndex > 0 ? sortedActiveFolderWords[viewingIndex - 1] : null;
  const nextWord = viewingIndex >= 0 && viewingIndex < sortedActiveFolderWords.length - 1
    ? sortedActiveFolderWords[viewingIndex + 1]
    : null;
